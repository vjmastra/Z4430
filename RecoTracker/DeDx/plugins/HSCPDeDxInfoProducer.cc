...
#include "RecoTracker/DeDx/plugins/HSCPDeDxInfoProducer.h"
...
void HSCPDeDxInfoProducer::produce(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  ...
  if(useTrajectory)iEvent.getByLabel(m_trajTrackAssociationTag, trajTrackAssociationHandle);
...
void HSCPDeDxInfoProducer::processHit(const TrackingRecHit* recHit, float trackMomentum, float& cosine, susybsm::HSCPDeDxInfo& hscpDeDxInfo,  LocalPoint HitLocalPos){
      ...
      if(clus.isPixel()){
          ...
          auto& detUnit     = *m_tracker->idToDetUnit(DetId((recHit->geographicalId())));
          float pathLen     = detUnit.surface().bounds().thickness()/fabs(cosine);
          float chargeAbs   =  pixelHit->cluster()->charge();
          ...
          auto& detUnitM     = *m_tracker->idToDetUnit(DetId((matchedHit->monoId())));
          int   NSaturating = 0;
          ...
          auto& detUnitS     = *m_tracker->idToDetUnit(DetId((matchedHit->stereoId())));
          NSaturating = 0;
